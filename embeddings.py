...
def main():
    ...
    if final_query:
        ...
        st.session_state.chat_history.append({
            ...
            "raw_cypher_result": raw_cypher_result, # <--- NEW FIELD
    ...
    for msg in st.session_state.chat_history:
        ...
        if msg["role"] == "assistant":
            ...
            # <--- ADD THIS LINE to retrieve the raw results --->
            raw_cypher_result = msg.get("raw_cypher_result", [])

            # 💡 NEW: Retrieve the context mode used for this specific answer
            ...
            if format_result or source_documents:
                with st.expander("Show Context & Cypher Results"):
                    ...
                    # ✅ NEW CONDITION: Only display if the result is meaningful AND 
                    # the mode was either "Cypher Query Only" or "Both"
                    ...
                    if should_display_cypher:
                        st.subheader("💡 Cypher Query Result (Primary Grounding Data)")
                        st.markdown(f"The intent was classified as `{intent}`, and the resulting Neo4j query provided the following specific data:")
                        # <--- REPLACE st.code with st.dataframe for structured view --->
                        if raw_cypher_result:
                            try:
                                # Convert the list of dicts to a DataFrame
                                df_result = pd.DataFrame(raw_cypher_result) 
                                st.dataframe(df_result) # Display the DataFrame beautifully
                            except Exception:
                                # Fallback to the original text view if conversion fails
                                st.code(format_result, language='text')
                        else:
                            # If no raw result, show the LLM-formatted string as a fallback
                            st.code(format_result, language='text')

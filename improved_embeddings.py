...
PREDEFINED_QUESTIONS = [
    ...
    "what is the number of flights that have the route from LAX to IAX ?",
    "Find flights from LAX to IAX with minimal delays?",
    "what are the destnations from LHX ?",
...
def extract_intent_and_entities(user_query):
    ...
    format_result = format_data_for_llm(result_cypher, intent,args)
    print("\nFormatted Result:", format_result)

    neo.CloseConnection()
    return format_result, intent , query_itself, result_cypher
...
def format_data_for_llm(data_list, intent, arguments=None):
    """
    ...
    if arguments:
        ...
        arg_string = ", ".join([f"{k}: {v}" for k, v in filtered_args.items()]) if filtered_args else "no specific filters"
    else:
        arg_string = "no specific filters"
    
    if not data_list:
        return f"No results found for the query with intent '{intent}' and arguments ({arg_string})."
    
    if isinstance(data_list, dict) and 'error' in data_list:
        return f"Query execution failed for intent '{intent}' with arguments ({arg_string}): {data_list['error']}"
    
    if not isinstance(data_list, list):
        return f"Invalid query result format for intent '{intent}' with arguments ({arg_string})."
    ...
    if not formatted_dicts:
        return f"No valid results found for the query with intent '{intent}' and arguments ({arg_string})."
        
    final_string = "; ".join(formatted_dicts) + "."
    return f"The results of the query with intent '{intent}' and arguments ({arg_string}) are: {final_string}"

# Assuming all helper functions (extract_intent_and_entities, format_data_for_llm, etc.) 
# and imports are correct and available.

def main():
    st.set_page_config(page_title="✈️ Airline Assisstant Chatbot -✈️ كل الطرق تؤدي الى روما")
    st.title("✈️ Airline Assisstant Chatbot -✈️ كل الطرق تؤدي الى روما")
    ...
    if final_query:
        ...
        # --- Process user query ---
        start_time = time.time() # Initialize timer here

        # 1. Intent and Cypher Query Execution
        format_result, intent , query_itself, raw_cypher_result = extract_intent_and_entities(user_query)

        with st.spinner(f"Generating answer using {selected_model_name}..."):
            try:
                # 2. Vector Retrieval (Required for 'vector_only' and 'both')
                if selected_context_mode in ["vector_only", "both"]:
                    retrieved_docs = retriever.invoke(user_query)
                    vector_structured = format_retrieved_docs_to_structured(retrieved_docs, add_prefix=False)
                else:
                    retrieved_docs = []
                    vector_structured = "N/A"
                ...
                if selected_context_mode == "cypher_only":
                    ...
                    result = chain.invoke({"input": user_query, "vector_context": vector_structured})
                    answer = result.content if hasattr(result, 'content') else str(result)
                ...
                # st.error(f"An error occurred during chain execution: {e}") # Do not show error here, just log
                answer = "Error generating response."
                source_documents = []
                latency = 0.0

        # >>> CRITICAL FIX 1: APPEND NEW Q&A TO CHAT HISTORY <<<
        ...
        st.session_state.chat_history.append({
            ...
            "raw_cypher_result": raw_cypher_result,
            "latency": latency,
    ...
    # >>> CRITICAL FIX 2: MOVE CHAT DISPLAY LOOP OUTSIDE OF THE `if final_query:` BLOCK <<<
    ...
    for msg in st.session_state.chat_history:
        ...
        if msg["role"] == "assistant":
            ...
            raw_cypher_result = msg.get("raw_cypher_result", [])
            context_mode_used = msg.get("context_mode", "")
            ...
            if format_result or source_documents:
                with st.expander("Show Context & Cypher Results"):
                    ...
                    should_display_cypher = (
                        is_cypher_result_meaningful and 
                        context_mode_used in ["Cypher Query Only", "Both (Cypher + Vector)"]
                    )
                    
                    if should_display_cypher:
                        ...
                        if raw_cypher_result:
                            try:
                                df_result = pd.DataFrame(raw_cypher_result) 
                        ...
                        if query_itself and query_itself != "N/A":
                            ...
                            st.code(query_itself, language='cypher')

                    # Vector Store Context
                    if source_documents and context_mode_used in ["Vector Store Only", "Both (Cypher + Vector)"]:
                        ...
                        # 1. Structured Data Passed to LLM (The combined context)
                        # NOTE: The helper function format_retrieved_docs_for_display must be defined elsewhere
                        vector_structured_context = format_retrieved_docs_to_structured(source_documents)
                        
                        st.markdown("#### 🤖 Structured Data Passed to LLM:")
                        st.info("This is the formatted text that was provided to the LLM as the `{context}` variable:")
                        st.code(vector_structured_context, language='text')

                        # 2. Natural Language Text Matched by Embeddings (Individual documents)
                        st.markdown("#### 🔍 Natural Language Text Matched by Embeddings:")
                        st.info("These are the semantic descriptions found most similar to your query:")
                        
                        with st.expander("View All Matched Semantic Descriptions", expanded=False):
                            for i, doc in enumerate(source_documents, 1):
                                ...
                                # Structured Data Record (Metadata)
                                record_data = doc.metadata.get('properties', {})
                                if record_data:
                                    st.markdown("**B. Structured Data Record (Metadata):**")
                                    st.json(record_data)
                                else:
